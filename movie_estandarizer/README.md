...
**Current Implementation**: CSV files are generated by executing the SQL queries located in each project folder. The workflow involves:
...
3. **Local Processing**: Place CSV in the appropriate folder and run Python scripts locally

**Future Roadmap**: The code is designed to be easily modified for **Snowflake Notebook integration** using Snowflake connectors. This will enable:
...
The current local implementation serves as a foundation that can be seamlessly migrated to cloud-native Snowflake execution.
